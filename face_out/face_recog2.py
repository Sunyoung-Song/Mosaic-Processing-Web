...
imgTest = face_recognition.load_image_file('img/btob2.jpg')    # test이미지를 바꾸고싶으면 여기를 바꾸기
...
cv2.rectangle(imgIU, (faceLoc[3], faceLoc[0]), (faceLoc[1], faceLoc[2]), (255, 0, 255), 2)    # cv2.rectangle(img, start, end, color, thickness)
...
cv2.imwrite('./downloads/res.jpg', imgTest)    # 이미지 저장
cv2.waitKey(0)    # 1ms안에 코드 종료됨