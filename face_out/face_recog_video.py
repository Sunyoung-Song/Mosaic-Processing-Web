...
def input1(video_path):
    # 학습할 이미지를 RGB형태로 변경
    imgTrain = face_recognition.load_image_file(video_path)
    imgTrain = cv2.cvtColor(imgTrain, cv2.COLOR_BGR2RGB)
    return imgTrain

def input2(video_path):
    # 테스트할 동영상을 불러옴
    test_video = video_path   # 테스트 비디오를 바꾸고싶다면 여기를 바꾸기
    cap=cv2.VideoCapture(test_video)
    return cap



def process(cap, imgTrain, path, path2):
    '''
    STEP 2:
    ...
    encodeimg = face_recognition.face_encodings(imgTrain)[0]    # return 128 measurement
    ...
    #writer = cv2.VideoWriter('./ImageBasic/face_mosaic.mp4', fourcc, fps, (w, h))
